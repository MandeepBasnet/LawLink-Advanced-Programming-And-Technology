...
/**
 * Servlet to handle confirmation of appointments by lawyers
 */
@WebServlet("/lawyer/confirm-appointment")
public class ConfirmAppointmentServlet extends HttpServlet {
    private static final Logger LOGGER = Logger.getLogger(ConfirmAppointmentServlet.class.getName());
    ...
    /**
     * Handles POST requests to confirm an appointment
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (appointment.getLawyerId() != user.getUserId()) {
                LOGGER.log(Level.WARNING, "User ID {0} attempted to confirm appointment ID {1} not owned by them", new Object[]{user.getUserId(), appointmentId});
                session.setAttribute("error", "You are not authorized to confirm this appointment.");
                response.sendRedirect(request.getContextPath() + "/lawyer/lawyer-dashboard");
                return;
            }

            // Check status
            if ("CANCELLED".equals(appointment.getStatus()) || "COMPLETED".equals(appointment.getStatus()) || "CONFIRMED".equals(appointment.getStatus())) {
                LOGGER.log(Level.WARNING, "Cannot confirm appointment ID {0} with status: {1} for user: {2}", new Object[]{appointmentId, appointment.getStatus(), user.getEmail()});
                session.setAttribute("error", "This appointment cannot be confirmed as it is already " + appointment.getStatus().toLowerCase() + ".");
                response.sendRedirect(request.getContextPath() + "/lawyer/lawyer-dashboard");
                return;
            }
            ...
            if (success) {
                LOGGER.log(Level.INFO, "Appointment ID {0} confirmed successfully by user ID {1}", new Object[]{appointmentId, user.getUserId()});
                session.setAttribute("success", "Appointment confirmed successfully.");
            } else {
                LOGGER.log(Level.SEVERE, "Failed to confirm appointment: {0} for user: {1}", new Object[]{appointmentId, user.getEmail()});
                session.setAttribute("error", "Failed to confirm appointment due to a server error.");
            }

            response.sendRedirect(request.getContextPath() + "/lawyer/lawyer-dashboard");

        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Unexpected error while confirming appointment for user: {0}", user.getEmail());
            session.setAttribute("error", "An error occurred while confirming the appointment.");
            response.sendRedirect(request.getContextPath() + "/lawyer/lawyer-dashboard");
        }
    }
}